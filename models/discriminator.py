...
class Discriminator(nn.Module):
    ...
    def forward(self, data, direct):
        ...
        for t in range(dimension):
            ...
            x_h = self.hist_reg(h)
            x_loss += torch.sum(torch.abs(m - x_h) * m) / (torch.sum(m) + 1e-5)

            z_h = self.feat_reg(x_h)
            x_loss += torch.sum(torch.abs(m - z_h) * m) / (torch.sum(m) + 1e-5)

            alpha = self.weight_combine(torch.cat([gamma_x], dim=1))

            c_h = alpha * z_h + (1 - alpha) * x_h
            x_loss += torch.sum(torch.abs(m - c_h) * m) / (torch.sum(m) + 1e-5)

            inputs = torch.cat([c_h], dim=1)
        ...
        return {'loss': x_loss * self.impute_weight + y_loss * self.label_weight, 'g_d_loss': y_loss}
